...
contract("ColonyNetworkMining", accounts => {
  ...
  describe("disagreement over child reputation updates", () => {
    ...
    it.only("if a child reputation calculation is wrong, it should be handled correctly if that user has never had that reputation before", async () => {
      await giveUserCLNYTokensAndStake(colonyNetwork, MAIN_ACCOUNT, "1000000000000000000");
      await giveUserCLNYTokensAndStake(colonyNetwork, OTHER_ACCOUNT, "1000000000000000000");
  ...
  describe("Intended ('happy path') behaviours", () => {
    ...
    it("should cope if someone's reputation would overflow, setting it to the maximum value instead", async () => {
      ...
      await goodClient.insert(globalKey, new BN("2").pow(new BN("128")).subn(2), 0);
      await goodClient.insert(userKey, new BN("2").pow(new BN("128")).subn(2), 0);
      await badClient.insert(globalKey, new BN("2").pow(new BN("128")).subn(2), 0);
      await badClient.insert(userKey, new BN("2").pow(new BN("128")).subn(2), 0);
