...
class ReputationMiner {
  ...
  async addLogContentsToReputationTree(blockNumber = "latest") {
    ...
    for (let i = ethers.utils.bigNumberify("0"); i.lt(totalnUpdates); i = i.add(1)) {
      await this.addSingleReputationUpdate(i, repCycle, blockNumber);
    }
  ...
  async addSingleReputationUpdate(updateNumber, repCycle, blockNumber) {
    ...
    if (updateNumber.lt(this.nReputationsBeforeLatestLog)) {
      ...
      if (reputation.gt(ethers.utils.bigNumberify("2").pow(128).sub(1).div(denominator))) {
      ...
      amount = this.getAmount(updateNumber, logEntry[1]);
            ...
            if (!originReputationValue.isZero()) {
              ...
              if (childSkillKeyExists) {
                ...
                if (absAmount.gt(originReputationValue)) {
                  ...
                  const absTargetAmount = targetAmount.mul(-1);
                  targetAmount = absTargetAmount.gt(childSkillReputation) ? childSkillReputation.mul(-1) : targetAmount;
                }
                amount = targetAmount;
              } else {
